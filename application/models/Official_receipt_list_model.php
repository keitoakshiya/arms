...
class official_receipt_list_model extends CI_Model {
    ...
    public function get_receipt($a){
        $this->db->select(' `receipt`.`id` AS `receipt_id`,
           ...
           IF (`receipt`.`or_amount`-IFNULL((SELECT (SUM(hospital_bill_payment)+SUM(professional_bill_payment)) FROM transaction WHERE receipt_id = receipt.id), 0)!=0,"Unapplied","Applied") as applied_stat
           ');
