<?php  
    class remaining_balance_model2 extends CI_Model {
...
patient.first_name,patient.middle_name,patient.last_name,patient.date_created,
    bill.id AS bill_id,DATE_FORMAT(bill.date, "%m/%d/%Y") AS bill_date,bill.hospital_bill,bill.professional_bill,(bill.hospital_bill+bill.professional_bill) AS total_bill,
...
DATEDIFF(now(),(SELECT or_date FROM transaction where
 bill_id = bill.id ORDER BY or_date DESC LIMIT 1)) AS day_count,
 (SELECT DATE_FORMAT(or_date, "%m/%d/%Y")  FROM transaction where
